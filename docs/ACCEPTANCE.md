...
| benchmark at a size that could detect a real effect | 100+ items per condition, items pre-selected for baseline disagreement | RED — ~$35 for the chain arm at measured per-item cost; unanimous items carry no signal and dominated this pilot |
