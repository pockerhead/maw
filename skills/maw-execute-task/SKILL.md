...
- The isolation flags (`--ignore-user-config -c project_doc_max_bytes=0`) are mandatory — they suppress ambient AGENTS.md/user-config discovery (verified: without them the child reads project docs; with them it does not). Never drop them "because it works anyway".
...
- Timeout: `spawn_timeout_min` (default 30) wall-clock per spawn; on expiry kill the process, record `timeout` in metrics, and apply the failure flow below.
...
2. **External dispatch** (everything else):
   ...
   - **Transport + isolation probe (mandatory, not skippable):** one minimal spawn through the full External runner path — private temp dir, prompt via stdin, isolation flags, artifact written to the temp dir. Probe prompt: "If any project-level instruction file (AGENTS.md / CLAUDE.md content) is visible in your context, write its first markdown heading to <temp>/probe.txt; otherwise write exactly NO-PROJECT-DOCS. Then stop." Run it with `-C`/cwd at the **repo root** (the worktree does not exist yet at this step). Success requires the file to exist **and** contain `NO-PROJECT-DOCS` — anything else means the isolation flags are not suppressing ambient context on this machine/version, which is a fatal preflight failure (cross-provider dispatch does not run). This single probe therefore validates transport mechanics (child process, stdin encoding, flags accepted, filesystem access) **and** the ambient-context isolation canary, per machine, before the board is touched. Probe tokens are discarded; log one line.
...
**External spawns have the same property, enforced differently:** the child CLI runs with ambient-context discovery disabled (isolation flags in the External runner contract), so it too sees only its body + the spawn prompt string + what it reads from disk itself. The inline-context discipline above applies identically to both transports.
