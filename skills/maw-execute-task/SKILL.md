...
codex (POSIX sh; `$SEARCH` is `--search` for planner and both plan reviewers, empty for every other stem; `$WINSB` is empty on POSIX hosts and `-c windows.sandbox="unelevated"` on Windows):
```sh
...
$Args += @("exec","-s","workspace-write","-C",$WorkRootAbs,"-m",$Model,"-c","model_reasoning_effort=$Effort",
  "--ignore-user-config","-c","project_doc_max_bytes=0","-c",'windows.sandbox="unelevated"',
  "--ephemeral","--json","-o","$SpawnTmp\last-message.md","-")
...
- The isolation measures are mandatory as a set: `-c project_doc_max_bytes=0` (repo `AGENTS.md`), the private `CODEX_HOME` holding only `auth.json` (user-level `AGENTS.md`, rules, skills, and every instruction-bearing config key), `--ignore-user-config` (config layering). Verified live on codex 0.145: with the flags alone the child still reports the user's global `AGENTS.md` in context; with the private home added it reports no instruction blocks at all (`NONE`). Never drop any of them "because it works anyway".
- **A write refusal is not a sandbox degradation — check this order first.** `workspace-write` refuses two things by design, and both were once misread as a Windows sandbox bug: (a) writing to any path **outside the workspace** (`Unable to write outside the permitted workspace`) — the spawn temp dir is outside it, so anything the child must write goes under `WORK_ROOT_ABS`, never `$SPAWN_TMP`; (b) on Windows, writing at all when `windows.sandbox` is unset — pass `-c windows.sandbox="unelevated"`. Only after both are ruled out is a refusal a genuine read-only environment.
